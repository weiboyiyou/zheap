...
/*
 * Similar to do_convert_tuple, but for zheap tuples.
 */
ZHeapTuple
do_convert_ztuple(ZHeapTuple tuple, TupleConversionMap *map)
{
	AttrNumber *attrMap = map->attrMap;
	Datum	   *invalues = map->invalues;
	bool	   *inisnull = map->inisnull;
	 ...
	 * this exactly matches the numbering convention in attrMap.
	 */
	zheap_deform_tuple(tuple, map->indesc, invalues + 1, inisnull + 1);
	 ...
	 * Now form the new tuple.
	 */
	return zheap_form_tuple(map->outdesc, outvalues, outisnull);
}
